...
import type { NodeStatus } from "@/src/canvas/components/node-status-badge";
import { BaseNodeData } from "@/src/canvas/nodes/nodes/node-types";
...
export const ClusterNode = memo(
  ({ data, isConnectable }: NodeProps<ClusterNodeData>) => {
    ...
    const handleResetSpatialClustering = () => {
      // NOTE: withActiveViewer was removed - this node is deprecated
      // withActiveViewer(viewer => {
      //   if (viewer && typeof viewer.resetSpatialClustering === 'function') {
      //     viewer.resetSpatialClustering();
      //   }
      // });
      console.warn('Cluster reset not available - deprecated node');
    };
